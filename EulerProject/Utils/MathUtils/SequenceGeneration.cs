...
public static class SequenceGeneration
{
    ...
    public static IEnumerable<int> Pandigitals(params int[] integers)
    {
...
{string.Join("; ", integers.Select((e, i) => (Index: i, Number: e)).Where(e => e.Number is < 0 or > 9))}");
        ...
        return PermutationUtils.Permute(string.Join(string.Empty, integers.ToHashSet())).Select(int.Parse);
    }

    public static IEnumerable<int> Pandigitals(int n, bool zeroless = false)
    {
        if (n < 1)
            throw new ArgumentException($"n must be equal or greater than 1 (value given was: {n})");

        return Pandigitals(Enumerable.Range(zeroless ? 1 : 0, n).ToArray());
    }

    public static IEnumerable<int> PandigitalPrimes(int n, bool zeroless = false) =>
